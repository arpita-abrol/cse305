...
public class MovieDao {
	...
	public List<Movie> getSummaryListing(String searchKeyword) {
		 ...
		 * to fetch details of summary listing of revenue generated by a particular
		 * movie or movie type must be implemented Each record is required to be
		 * encapsulated as a "Movie" class object and added to the "movies" ArrayList
		 * Store the revenue generated by an movie in the soldPrice attribute, using
		 * setSoldPrice method of each "movie" object
		 */
	...
	public List<Movie> getMovieSuggestions(String customerID) {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://mysql3.cs.stonybrook.edu:3306/" + System.getenv("NETID"), System.getenv("NETID"), System.getenv("SBUID"));
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("Call suggested("+customerID+")");
			while(rs.next()) {
				Movie movie = new Movie();
...
//		}
	...
	public List<Movie> getCurrentMovies(String customerID){
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://mysql3.cs.stonybrook.edu:3306/" + System.getenv("NETID"), System.getenv("NETID"), System.getenv("SBUID"));
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("SELECT * FROM viewcurrentmovies WHERE CustID="+customerID);
			while (rs.next()) {
				Movie movie = new Movie();
...
public List<Movie> getQueueOfMovies(String customerID){
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://mysql3.cs.stonybrook.edu:3306/" + System.getenv("NETID"), System.getenv("NETID"), System.getenv("SBUID"));
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("SELECT * FROM viewmoviequeue where CustID="+customerID);
			while (rs.next()) {
				Movie movie = new Movie();
				movie.setMovieID(rs.getInt("MovieID"));
				movie.setMovieName(rs.getString("MovieName"));
...
//	}
	...
	public List getMoviesByActor(String actorName) {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://mysql3.cs.stonybrook.edu:3306/" + System.getenv("NETID"), System.getenv("NETID"), System.getenv("SBUID"));
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("SELECT * FROM viewmoviesbyactor where ActorName LIKE '%"+actorName +"%'");
			while (rs.next()) {
				Movie movie = new Movie();
				movie.setMovieID(rs.getInt("MovieID"));
				movie.setMovieName(rs.getString("MovieName"));
